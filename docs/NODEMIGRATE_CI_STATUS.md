...
| 2026-09-25 | `217bf87275f1abbacdcb1f6e587b5914f72f3f92` | Branch-runtime migration and Docker preflight | `nodemigrate` and combined branch-runtime build steps passed; five-node Docker isolation preflight passed. Both K3s and upstream `Run migration` steps failed. Completed job metadata is available, but the GitHub API became unreachable before artifact/job logs could be retrieved, so the failure stage is not yet verified. | [Run 36124385324](https://github.com/centerionware/not-k8s/actions/runs/36124385324); log retrieval pending |
| 2026-09-25 | Working tree after `217bf872` | Expanded migration fixture follow-up | Adds ClusterRole/ClusterRoleBinding Node-read authorization, ResourceQuota, LimitRange, PriorityClass scheduling state, live ConfigMap/Secret consumption, and Cilium Helm release/value/manifest checks plus a pinned server-side dry-run upgrade. Shell syntax, snapshot-filter, and whitespace checks passed. Runtime validation is pending; embedded YAML parsing was skipped because PyYAML is not installed. | Pending GitHub connectivity |
