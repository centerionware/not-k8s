...
| Follow-up to `c56d3f3ecce3a43339ba0df23aec02a36193d841` | nodeapiserver map-key CEL | Treat typed additionalProperties schemas as CEL maps and cost single-variable map comprehensions by bounded key iteration. Add type and static-cost regressions for Gateway API annotation rules. Source evidence is resolved; focused quick-check and branch/runtime-source retests are pending. | Worktree changes; not dispatched yet |
