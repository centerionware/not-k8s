...
| Follow-up to `f2d2c7639b78ee1a1cb9b6609903e6374fb24716` | HTTPRoute condition wait correction | Replace unsupported top-level `kubectl wait` checks with a bounded `jq` poll over each route's parent conditions, and print the route if the expected condition times out. Shell/snapshot/diff checks and migration rerun pending. | Not dispatched yet |
