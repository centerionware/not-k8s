...
| 2026-09-25 | `64ea1fa3f0975b4087b4f2b126cd8996c53f23b5` | Branch-runtime K3s+Cilium and upstream+Cilium | Both lanes passed source resource setup, PVC/data checks, workloads, accepted/programmed Gateway API conditions, and allowed/denied RBAC probes. Both failed when the Traefik Ingress/Gateway traffic probe did not return the nginx page. `nodemigrate` was not invoked. Utility/runtime builds and Docker five-node preflight passed. | [Run 36138657733](https://github.com/centerionware/not-k8s/actions/runs/36138657733); logs `/tmp/nodemigrate-36138657733/` |
| Follow-up to `64ea1fa3f0975b4087b4f2b126cd8996c53f23b5` | Traefik traffic probe diagnostics | Capture HTTP statuses/bodies, Service and EndpointSlice state, route details, and Traefik logs on probe failure. The goal document explicitly extends coverage to additional discovered API kinds. Local shell, snapshot, and diff checks passed; migration rerun pending. | Not dispatched yet |
