...
| 2026-09-25 | `6a2e99d6b205626ae125e231d276c1936c3f1986` | Branch-runtime K3s+Cilium and upstream+Cilium | Utility/runtime builds and five-node Docker preflight passed. Both source fixtures passed. Destination import accepted 57 K3s / 53 upstream CRD apply requests, with one failed CRD in each lane: only `tlsroutes.gateway.networking.k8s.io` remained rejected because CEL could not estimate the bounded `hostname.substring(2).matches(...)` result. The other previously failing Gateway CRDs now passed. Upstream also failed CertificateRequest admission because its cert-manager webhook remained unreachable during target Cilium startup. Both lanes rolled back, restored the source API, and retained protected exports. | [Run 36148384987](https://github.com/centerionware/not-k8s/actions/runs/36148384987); artifacts `/tmp/nodemigrate-36148384987/artifacts/` |
| Follow-up to `6a2e99d6b205626ae125e231d276c1936c3f1986` | nodeapiserver bounded substring cost | Propagate the source string maximum to `substring` result cost and add a TLSRoute-style bounded hostname regression. The map-key compatibility fix is partially confirmed: three CRD failures fell to one. The focused nodeapiserver check passes at `8555845f`; substring fix and runtime retest are pending. | Current worktree; migration evidence [36148384987](https://github.com/centerionware/not-k8s/actions/runs/36148384987) |
