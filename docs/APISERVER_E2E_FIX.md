...
| Nodeproxy headless/ClusterIP routing | in progress | Focused run `33592332954` still timed out before normal ClusterIP traffic reached its backend. Its compact audit records show only the repeated EndpointSlice 404s after the test timeout/namespace cleanup, while the selected failure snapshot still had no test ClusterIP nft rule and nodeproxy had no watch error. Earlier diagnostics proved the EndpointSlice PATCH header and SSA classification; they did not yet prove whether the initial apply was persisted, cached, or delivered to nodeproxy. PR #497 remains open while that initial path is traced. | [#497](https://github.com/centerionware/not-k8s/pull/497) |
