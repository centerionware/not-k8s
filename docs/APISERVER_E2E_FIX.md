...
| 3 | Cert-manager ServiceAccount token requests and CRD usability | Verified failure on clean nodeapiserver base: the API stayed responsive, CoreDNS token requests returned 201, but all three cert-manager ServiceAccount TokenRequests returned 404 and deployment readiness timed out. The direct post-install ServiceAccount assertion passes; the focused test now checks a bound token for a live cert-manager Pod to isolate the remaining path. |
...
| Cert-manager ServiceAccount token requests / CRD usability | confirmed failure; root cause isolation in progress | Clean-base run `33601140162` served the nodeapiserver and remained responsive, but cert-manager's `cert-manager`, `cert-manager-cainjector`, and `cert-manager-webhook` projected-token requests all returned HTTP 404 while CoreDNS's identical token path returned 201. The test timed out on deployment readiness before reaching the CRD exercise. PR #498's direct post-install assertion confirmed all three ServiceAccounts are persisted; its next focused assertion checks a bound token against a live cert-manager Pod. | [#498](https://github.com/centerionware/not-k8s/pull/498) |
