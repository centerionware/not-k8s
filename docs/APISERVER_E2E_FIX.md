...
| Dry-run collection delete semantics | in progress | The dedicated `deletecollection` listener path listed and admitted objects but called the non-dry-run delete helper, so `dryRun=All` still persisted deletions. The existing e2e test reproduces this with a ConfigMap; the fix forwards the parsed dry-run option through admission and `delete_with_options`. | [pending] |
