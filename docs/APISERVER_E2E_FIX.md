...
| 3 | Cert-manager ServiceAccount token requests and CRD usability | Verified failure on clean nodeapiserver base: the API stayed responsive, CoreDNS token requests returned 201, but all three cert-manager ServiceAccount TokenRequests returned 404 and deployment readiness timed out. The next isolated PR adds a direct post-install ServiceAccount assertion before changing API code. |
