...
| Nodeproxy headless/ClusterIP routing | in progress | Focused runs `33580340080` and `33584791114` kept both test EndpointSlices absent: nodecontroller's apply-shaped PATCH fell through to ordinary PATCH and received HTTP 404 because the object did not yet exist, while an existing-object apply for `kube-dns` succeeded. The diagnostic namespace warning appeared only during cleanup after the test namespace was deleted. PR #497 routes the apply-only `fieldManager` + `force` request shape through nodeapiserver's create-on-apply path even when the media-type header is absent; its focused e2e rerun is pending. | [#497](https://github.com/centerionware/not-k8s/pull/497) |
