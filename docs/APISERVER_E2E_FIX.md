...
| DRA claim allocation and workload startup | open | Earlier shard 2 DRA/raw-block/fsGroup timeouts were observed alongside broader startup failures. API discovery now passes; run `test_dra_claim_is_allocated_and_reserved_for_the_pod` independently before attributing allocation or workload failures to nodeapiserver. | — |
