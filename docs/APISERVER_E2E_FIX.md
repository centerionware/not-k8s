...
| Nodeproxy headless/ClusterIP routing | in progress | Focused runs `33580340080`, `33584791114`, and `33588192416` still timed out with the test EndpointSlices absent. The first diagnostic run showed the original 404s were ordinary PATCH missing-object responses; PR #497 now routes the apply-only `fieldManager` + `force` request shape through create-on-apply when the media-type header is absent, and the latest diagnostic build logs the actual request classification for the next investigation. | [#497](https://github.com/centerionware/not-k8s/pull/497) |
