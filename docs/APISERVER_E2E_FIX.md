...
| API listener does not recover after environment reconfiguration | in progress | Shards 1–5 lose API availability during or after authorization/audit reconfiguration tests; later requests fail with connection refused or recovery timeout. The recovery barrier already exists but the newly-added authorization-webhook test was not classified as disruptive, and recovery errors were only logged. This PR classifies it and makes the barrier fail the focused run. | pending |
