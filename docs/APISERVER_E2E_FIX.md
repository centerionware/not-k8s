...
| Termination grace period | in progress | The current Pod delete path physically removed ordinary Pods immediately and did not pass `DeleteOptions.gracePeriodSeconds` into REST. The fix applies the upstream Pod strategy: use an explicit grace period or the scheduled Pod's `terminationGracePeriodSeconds`, persist `deletionTimestamp`/`deletionGracePeriodSeconds`, and let nodelet issue the final zero-grace delete. The existing e2e test covers the real runtime behavior. | [pending] |
