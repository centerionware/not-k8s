...
| 3 | Cert-manager ServiceAccount token requests and CRD usability | The follow-up bound TokenRequest for a live cert-manager Pod passed on both the upstream comparison and explicit nodeapiserver runs, while both filtered runs still timed out waiting for cert-manager deployments. This validates the API path after the Pod exists; the remaining startup failure is not attributable to nodeapiserver. |
...
| Cert-manager ServiceAccount token requests / CRD usability | confirmed startup failure; API path verified | The initial comparison run `33607284183` used the workflow's explicit default `apiserver=upstream`; the explicit nodeapiserver run `33609364213` produced the same deployment-readiness timeout. Both runs confirmed the ServiceAccounts exist and the bound TokenRequest for a live cert-manager Pod succeeds, so the remaining startup failure does not justify a nodeapiserver source change. | [#498](https://github.com/centerionware/not-k8s/pull/498) |
...
| `33601140162` | failed | Clean-base cert-manager run: nodeapiserver stayed responsive, but the three cert-manager ServiceAccount TokenRequests returned 404 while CoreDNS's token requests returned 201; deployment readiness timed out before the CRD-backed API checks. |
| `33607284183` | failed | PR #498 filtered cert-manager comparison run using the workflow default `apiserver=upstream`: build and bootstrap passed; the direct ServiceAccount checks and bound TokenRequest for a live cert-manager Pod passed, but cert-manager deployment readiness still timed out. |
| `33609364213` | failed | PR #498 filtered cert-manager run with `apiserver=nodeapiserver`, normal nodeproxy, and flannel: build and bootstrap passed; the test again reached the deployment-readiness timeout after the ServiceAccount and bound-token checks. No nodeapiserver source defect was reproduced. |
