...
| Nodeproxy headless/ClusterIP routing | in progress | Focused run `33580340080` kept both test EndpointSlices absent: nodecontroller's server-side-apply PATCH received HTTP 404 while the same existing-object apply path succeeded for `kube-dns`. The nodeproxy table therefore never received the test Service endpoints. The current branch is isolating the two nodeapiserver 404 paths (namespace admission versus create-on-apply resource handling) before changing behavior. | — |
