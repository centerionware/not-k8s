...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Branch-runtime run [36142617576](https://github.com/centerionware/not-k8s/actions/runs/36142617576) passed utility/runtime builds, Docker preflight, and expanded source checks including workload/storage/certs/Helm/RBAC/Ingress/Gateway traffic. Both invoked nodemigrate, then rolled back during import because nodeapiserver lacks the global CEL `matches` function used by Gateway API v1.6.1; upstream also returned HTTP 500 on CertificateRequest. | [CI status](NODEMIGRATE_CI_STATUS.md) |
