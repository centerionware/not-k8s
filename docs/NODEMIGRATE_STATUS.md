...
| Bugs found and component fixes | Confirmed runtime defects are tracked by owning component, including API-version negotiation, CSR ExtraValue codec, controller write identity/RBAC, failed-cutover rollback, omitted standalone Pods/rollout history, and Gateway API CEL compatibility. Global `matches` and typed map-key CEL fixes pass focused nodeapiserver checks; runtime testing reduced Gateway CRD failures to one TLSRoute substring-cost error, now being fixed. | [Bug and fix tracker](NODEMIGRATE_BUGS.md) |
...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36148384987](https://github.com/centerionware/not-k8s/actions/runs/36148384987) passed utility/runtime builds, Docker preflight, and source fixtures. Both invoked nodemigrate and rolled back: TLSRoute CEL substring-cost validation still rejects one CRD; upstream also failed CertificateRequest admission while the cert-manager webhook was unreachable during target Cilium startup. Neither target, reverse, nor parity checkpoint passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
