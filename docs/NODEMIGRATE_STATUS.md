...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest run passed full source checks and displayed the unattended risk warning in both lanes. K3s unexpectedly selected flannel instead of Cilium for target bootstrap; upstream reached nodeapiserver startup but port 6443 remained bound. A malformed YAML fixture in the new Tokio-context regression also needs a focused retest. | [CI status](NODEMIGRATE_CI_STATUS.md) |
