...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36131837328](https://github.com/centerionware/not-k8s/actions/runs/36131837328) passed utility/runtime builds, Docker preflight, fixture CPU/PVC/Pod readiness, Gateway, and nested HTTPRoute checks. Both source lanes then failed because the PriorityClass API omitted the default false `globalDefault` field. The assertion now handles that API default; migration rerun pending. Neither lane invoked nodemigrate. | [CI status](NODEMIGRATE_CI_STATUS.md) |
