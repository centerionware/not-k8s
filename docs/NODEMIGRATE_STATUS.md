...
| Workload and API-kind parity | Required fixture explicitly covers ConfigMaps, CRDs/custom resources, Deployments, StatefulSets, DaemonSets, Jobs/CronJobs, Helm release state, Ingress/Gateway API, RBAC, networking, storage, admission, and all listable discovered resources. Most behavior assertions and full round-trip comparisons remain unverified. | [Migration status](NODEMIGRATE_MIGRATION_STATUS.md) |
| Bugs found and component fixes | Confirmed runtime defects are tracked by owning component, including API-version negotiation, CSR ExtraValue codec, controller write identity/RBAC, failed-cutover rollback, omitted standalone Pods/rollout history, and Gateway API CEL compatibility. Global `matches`, map-key, and substring-cost fixes pass focused nodeapiserver checks; branch runtime accepts all source CRDs. Exact source-identity cleanup stopped two orphan Envoy processes and the prior socket bind error did not recur. Target Cilium `mount-cgroup` exits with code 1 and nodelet removes it; focused nodelet quick-check passed, and a live migration log watcher awaits verification. Upstream webhook import still fails. | [Bug and fix tracker](NODEMIGRATE_BUGS.md) |
...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime migration [36159532331](https://github.com/centerionware/not-k8s/actions/runs/36159532331) passed builds, Docker preflight, source fixtures, and exact-identity Cilium cleanup; the previous K3s Envoy `errno=98` did not recur. The target Cilium `mount-cgroup` init container repeatedly starts then exits code 1 and is removed before post-failure CRI inspection. Focused nodelet quick-check [36159523228](https://github.com/centerionware/not-k8s/actions/runs/36159523228) passed. A live init-log watcher was added after this run and awaits migration verification. Upstream accepted 54 CRD requests but CertificateRequest import returned 500 while the cert-manager webhook was unreachable; rollback retained the export. The latest-release comparison [36153673997](https://github.com/centerionware/not-k8s/actions/runs/36153673997) used K3s `v1.35.0+k3s1` and upstream Kubernetes `v1.37.1`; v0.8.0 rejected three Gateway API CRDs and upstream CertificateRequest/CSR import, then both lanes rolled back. No target checkpoint, reverse migration, or parity comparison has passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
