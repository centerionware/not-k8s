...
| Bugs found and component fixes | Run 36223445443 showed target Cilium/Traefik API watches failing TLS with `x509: certificate signed by unknown authority`; source root CA ConfigMaps were a strong stale-trust candidate. The exporter now skips this destination-managed ConfigMap. Run 36224872002 stopped before nodemigrate because the new source-stage CA assertion falsely reported mismatches; its shell PEM comparison lost trailing newlines. The assertion now compares encoded bytes. Focused nodemigrate tests passed at `cf1b984a`, but migration runtime has not exercised the exporter fix. The kind-wide omission of user-managed Endpoints, EndpointSlices, and application Leases is fixed at SHA `d4f3c4a0`; focused tests pass, runtime verification is pending. | [Bug and fix tracker](NODEMIGRATE_BUGS.md) |
...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest completed run [36224872002](https://github.com/centerionware/not-k8s/actions/runs/36224872002) passed scoped builds and Docker isolation but stopped at a faulty source-stage CA assertion before invoking nodemigrate. The assertion now compares encoded bytes; exporter runtime behavior remains unverified. Previous run [36223445443](https://github.com/centerionware/not-k8s/actions/runs/36223445443) showed TLS trust failures at the target API, which motivated skipping source `kube-root-ca.crt` during export. | [CI status](NODEMIGRATE_CI_STATUS.md) |
...
- Dedicated migration run [36224872002](https://github.com/centerionware/not-k8s/actions/runs/36224872002)
  passed both builds and Docker isolation, then stopped at a source-stage CA
  assertion before nodemigrate ran. The faulty PEM string comparison has been
  changed to exact base64-byte comparison. Focused nodemigrate tests passed at
  `cf1b984a` in [run 36224841930](https://github.com/centerionware/not-k8s/actions/runs/36224841930).
  The root-CA exporter fix has no runtime migration evidence yet; no target
  workload, reverse migration, or parity checkpoint passed.
