...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36138657733](https://github.com/centerionware/not-k8s/actions/runs/36138657733) passed utility/runtime builds, Docker preflight, source workloads, storage, Job/CronJob, RBAC probes, Gateway conditions, and fixture assertions. Both lanes failed source-stage Traefik traffic probes; nodemigrate was not invoked. The next run adds HTTP response and Traefik/service diagnostics. | [CI status](NODEMIGRATE_CI_STATUS.md) |
