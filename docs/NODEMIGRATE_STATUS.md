...
| Bugs found and component fixes | Confirmed runtime defects are tracked by owning component, including API-version negotiation, CSR ExtraValue codec, controller write identity/RBAC, failed-cutover rollback, omitted standalone Pods/rollout history, and Gateway API CEL compatibility. The global `matches` fix passed quick-check; bounded map-key CEL compatibility is being fixed from the next migration finding. | [Bug and fix tracker](NODEMIGRATE_BUGS.md) |
