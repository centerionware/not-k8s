...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36150670405](https://github.com/centerionware/not-k8s/actions/runs/36150670405) passed utility/runtime builds, Docker preflight, source fixtures, and all CRD imports. K3s forward migration completed, but target Envoy failed to bind its socket (`errno=98`) and CNI/CSI stayed unavailable. Upstream CertificateRequest admission failed because the cert-manager webhook was unreachable with target CNI down; that lane rolled back. Latest-release comparison [36153673997](https://github.com/centerionware/not-k8s/actions/runs/36153673997) passed source fixtures on K3s `v1.35.0+k3s1` and upstream Kubernetes `v1.37.1`, and emitted the noninteractive risk warning. The `v0.8.0` destination rejected three Gateway API CRDs in both lanes with undeclared CEL `matches`; upstream also returned 500 for CertificateRequest/CSR import. Both rolled back and retained protected exports. No target checkpoint, reverse migration, or parity comparison passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
