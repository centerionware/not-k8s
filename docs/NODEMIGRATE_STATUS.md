...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36126850738](https://github.com/centerionware/not-k8s/actions/runs/36126850738) passed utility/runtime builds and the five-node Docker preflight. Both single-node lanes failed during source fixture setup before invoking nodemigrate: the seed Pod stayed Pending, upstream events showed CPU pressure, and Traefik rejected the Gateway's port 80 listener. Fixture requests, Gateway port, and diagnostics are corrected in the current worktree; rerun pending. No migration or parity checkpoint passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
