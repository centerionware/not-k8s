...
| Bugs found and component fixes | Confirmed runtime defects are tracked by owning component: API-version negotiation, CSR ExtraValue codec, controller write identity/RBAC, failed-cutover rollback, and omitted standalone Pods/rollout history. Branch fixes and focused evidence are recorded separately. | [Bug and fix tracker](NODEMIGRATE_BUGS.md) |
...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36119449016](https://github.com/centerionware/not-k8s/actions/runs/36119449016) passed builds and the five-node Docker preflight. K3s forward import/API readiness passed, but target CNI and CSI checks failed; diagnostics used the stopped source kubeconfig, now corrected in the worktree. Upstream CertificateRequest webhook import failed with HTTP 500 and recovered source/export. No return migration or parity passed. Focused `nodebootstrap` check passed in [36120097438](https://github.com/centerionware/not-k8s/actions/runs/36120097438); migration rerun pending. | [CI status](NODEMIGRATE_CI_STATUS.md) |
