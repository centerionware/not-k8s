...
| Bugs found and component fixes | Job selector defaulting, migrated PVC bind-status recovery, and CSI staging path fixes passed focused checks. The latest migration run 36248706224 confirmed PVCs bind but exposed nodelet's missing hostPath/local PV mounting; the focused fix and rerun are pending. No return migration or parity checkpoint passed. | [Bug and fix tracker](NODEMIGRATE_BUGS.md) |
...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36248706224](https://github.com/centerionware/not-k8s/actions/runs/36248706224) passed preflight/builds and target checks through static PVC binding in both lanes, then timed out because nodelet did not mount a bound hostPath PV. HostPath/local PV fix and focused rerun are pending. No returned-source, parity, or round-trip checkpoint passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
