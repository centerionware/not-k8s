...
| Bugs found and component fixes | Latest branch run 36232994720 passed source checks and forward migration in both lanes. Both target stages stop because Traefik GatewayClass status writes repeatedly receive API 409 conflicts, leaving `Accepted=Unknown`. The underlying resourceVersion conflict is under investigation; no return migration or parity checkpoint passed. | [Bug and fix tracker](NODEMIGRATE_BUGS.md) |
...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36232994720](https://github.com/centerionware/not-k8s/actions/runs/36232994720) passed preflight/builds and forward migration in both lanes. K3s reached Ready with Cilium and fixture DaemonSet/nginx Deployment rolled out; both lanes then failed the GatewayClass Accepted check because Traefik status writes got persistent 409 responses. No returned-source, parity, or round-trip checkpoint passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
