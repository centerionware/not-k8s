...
| Bugs found and component fixes | Run 36234240173 traced the GatewayClass 409 to streaming-list WATCH initial events using collection RV for each object. The per-object RV fix and regression are in the branch; focused validation is pending. No return migration or parity checkpoint passed. | [Bug and fix tracker](NODEMIGRATE_BUGS.md) |
...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36234240173](https://github.com/centerionware/not-k8s/actions/runs/36234240173) passed preflight/builds and forward migration in both lanes, then failed GatewayClass Accepted because initial WATCH events carried a snapshot RV newer than each object's stored RV. Fix and regression are pending validation. No returned-source, parity, or round-trip checkpoint passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
