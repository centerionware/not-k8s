...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run at `ccb4d413` built nodemigrate and the runtime and passed the five-node Docker isolation preflight. K3s forward API import passed, but target Cilium stayed `PodInitializing` and CNI/CoreDNS/CSI checks failed; destination diagnostics now capture init output, but the selected agent Pod query returned `NotFound`. Upstream failed CertificateRequest import with HTTP 500 and recovered source/export. No return migration or parity passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
