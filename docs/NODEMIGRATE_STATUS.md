...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest migration run passed source setup, displayed the unattended risk warning, and bootstrapped the target in both lanes. API import failed because dynamic source list items lacked top-level type metadata; the exporter now restores it from discovery. | [CI status](NODEMIGRATE_CI_STATUS.md) |
