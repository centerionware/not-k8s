...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest run passed source setup, displayed the unattended risk warning, and bootstrapped both targets. Import now gets past missing type metadata but fails because destination discovery lacks source custom APIs (`VolumeSnapshotClass` in K3s; `CiliumEndpoint` upstream). | [CI status](NODEMIGRATE_CI_STATUS.md) |
