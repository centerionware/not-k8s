...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36136419993](https://github.com/centerionware/not-k8s/actions/runs/36136419993) passed utility/runtime builds, Docker preflight, source fixtures, Gateway/HTTPRoute, certificate, workloads, storage, Jobs, and CronJob checks. RBAC probes then stayed Pending with `Insufficient cpu`; they now have explicit small requests. Neither lane invoked nodemigrate yet. | [CI status](NODEMIGRATE_CI_STATUS.md) |
