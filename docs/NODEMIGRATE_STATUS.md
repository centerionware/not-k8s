...
| Bugs found and component fixes | Latest branch run 36231779729: K3s cutover stopped on a CRI sandbox stop timeout; upstream migration reached destination API readiness but the verifier failed on the unsupported `crd` kubectl shortcut. Fixes for both are in progress; the external-CNI API endpoint change is not directly verified by this run. | [Bug and fix tracker](NODEMIGRATE_BUGS.md) |
...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36231779729](https://github.com/centerionware/not-k8s/actions/runs/36231779729) passed preflight and builds. K3s failed stopping a source sandbox (`DeadlineExceeded`) and recovered source with export retained. Upstream migration reported destination API readiness, then stage verification failed because not-k8s does not advertise kubectl's `crd` shortcut. No round trip or parity gate passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
