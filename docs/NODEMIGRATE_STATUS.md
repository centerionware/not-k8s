...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36129138298](https://github.com/centerionware/not-k8s/actions/runs/36129138298) passed utility/runtime builds and five-node Docker preflight. Fixture CPU requests, seed Pod readiness, and Gateway programming succeeded. Both lanes stopped at a faulty top-level HTTPRoute condition wait; upstream status already showed `Accepted=True` and `ResolvedRefs=True` under parent conditions. The fixture now polls those nested conditions; migration rerun pending. No nodemigrate stage or parity checkpoint passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
