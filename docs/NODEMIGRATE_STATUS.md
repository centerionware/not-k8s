...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest migration run passed full source checks and displayed the unattended risk warning in both lanes. K3s target selection exposed stale CNI backup files being mistaken for active config; the detector now filters config extensions. Upstream reached nodeapiserver startup but port 6443 remained bound. The Tokio-context regression now passes its corrected fixture. | [CI status](NODEMIGRATE_CI_STATUS.md) |
