...
| Bugs found and component fixes | Confirmed defects remain tracked by owner. In run 36195385046, Cilium's `mount-bpf-fs` stop event had Pod metadata and CRI later showed successful exit, but Pod status stayed Running and CNI did not start; the run did not reproduce CiliumNode 409s. Direct Cilium-gate and CRI-init-state diagnostics passed focused nodelet check 36197970777; the two-lane diagnostic rerun 36197970992 is pending. | [Bug and fix tracker](NODEMIGRATE_BUGS.md) |
...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest completed branch-runtime run [36195385046](https://github.com/centerionware/not-k8s/actions/runs/36195385046) passed scoped builds and Docker isolation. K3s imported all 59 CRDs and reached API readiness, then stalled after successful `mount-bpf-fs` exit with stale Pod status; upstream imported all 55 CRDs, then lost target Cilium networking and failed cert-manager webhook admission. Both rollbacks retained exports. Focused nodelet check passed at 36197970777; diagnostic rerun 36197970992 is pending. Neither lane has passed target, reverse, or parity checks. | [CI status](NODEMIGRATE_CI_STATUS.md) |
