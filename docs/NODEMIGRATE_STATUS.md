...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest run passed source setup, displayed the unattended risk warning, and bootstrapped both targets. Import gets past missing type metadata but fails because destination discovery lacks cert-manager, Cilium, and snapshot APIs; K3s Addon, CertificateSigningRequest, and ClusterTrustBundle gaps also surfaced. | [CI status](NODEMIGRATE_CI_STATUS.md) |
...
| Docker five-node isolation preflight | The image built, but the first container exited with code 255 before systemd readiness without container output. The entrypoint now prints the resolved systemd binary and enables debug console logs while retaining private cgroup namespaces. Run 36051665474 reproduced the failure. Isolation, CRI, BPF, network, and failure/restart remain unverified. | [CI status](NODEMIGRATE_CI_STATUS.md) |
