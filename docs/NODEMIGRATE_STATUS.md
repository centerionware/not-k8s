...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | At utility SHA `459570ee`, both branch-runtime and v0.8.0 K3s lanes completed forward API import and destination readiness, but Cilium/CNI and CSI readiness failed before workload checks or return migration. Both upstream lanes failed CertificateRequest webhook access; v0.8.0 also reproduced the known CSR codec error. No round trip passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
