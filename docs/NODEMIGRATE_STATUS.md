...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime migration [36156187055](https://github.com/centerionware/not-k8s/actions/runs/36156187055) passed builds, Docker preflight, source fixtures, and exact-identity Cilium cleanup; the previous K3s Envoy `errno=98` did not recur. The target Cilium agent remains `Init:1/6` at `mount-cgroup`, leaving CNI/CSI unavailable and preventing the target checkpoint or reverse migration. Upstream accepted 54 CRD requests but CertificateRequest import returned 500 while the cert-manager webhook was unreachable; rollback retained the export. Focused nodemigrate quick-check [36156187333](https://github.com/centerionware/not-k8s/actions/runs/36156187333) passed. Latest-release comparison [36153673997](https://github.com/centerionware/not-k8s/actions/runs/36153673997) used K3s `v1.35.0+k3s1` and upstream Kubernetes `v1.37.1`; v0.8.0 rejected three Gateway API CRDs and upstream CertificateRequest/CSR import, then both lanes rolled back. No target checkpoint, reverse migration, or parity comparison has passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
