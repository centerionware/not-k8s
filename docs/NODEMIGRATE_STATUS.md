...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36133100159](https://github.com/centerionware/not-k8s/actions/runs/36133100159) passed utility/runtime builds, Docker preflight, fixture readiness, Gateway/HTTPRoute, PriorityClass, certificate, and workload checks. Both source lanes then failed on jq syntax for a hyphenated Secret key; bracket notation is corrected in the worktree. Neither lane invoked nodemigrate; rerun pending. | [CI status](NODEMIGRATE_CI_STATUS.md) |
