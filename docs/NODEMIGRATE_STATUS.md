...
| Bugs found and component fixes | Confirmed runtime defects are tracked by owning component, including API-version negotiation, CSR ExtraValue codec, controller write identity/RBAC, failed-cutover rollback, omitted standalone Pods/rollout history, and Gateway API CEL compatibility. Global `matches`, map-key, and substring-cost fixes pass focused nodeapiserver checks; branch runtime accepted all source CRDs in run 36192756836. K3s Cilium still exhausts ten CiliumNode update conflicts after destination API readiness; status-subresource stale-version diagnostics were added after this run and need quick-check plus rerun. Upstream target Cilium CNI/agent disappeared, leaving cert-manager webhooks unreachable. | [Bug and fix tracker](NODEMIGRATE_BUGS.md) |
...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | In branch-runtime migration [36192756836](https://github.com/centerionware/not-k8s/actions/runs/36192756836), both scoped builds and Docker preflight passed. K3s imported all 59 CRDs and reached destination API readiness, but Cilium exhausted ten CiliumNode 409 retries before target workload checks; upstream imported all 55 CRDs, then Cilium CNI/agent loss left the cert-manager webhook unreachable. Rollback retained the export. The earlier init-container lifecycle diagnostics and focused nodelet check are recorded below; neither lane has passed target, reverse, or parity checks. | [CI status](NODEMIGRATE_CI_STATUS.md) |
