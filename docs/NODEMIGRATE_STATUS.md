...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Branch-runtime run [36140783446](https://github.com/centerionware/not-k8s/actions/runs/36140783446) passed utility/runtime builds, Docker preflight, and source workloads/storage/certs/Helm/RBAC. Ingress returned nginx (HTTP 200); Gateway returned 404 because its probe used the Service's port 80→8000 rather than configured listener port 8080. The harness now probes the correct listener. `nodemigrate` has not yet run with the expanded fixtures. | [CI status](NODEMIGRATE_CI_STATUS.md) |
