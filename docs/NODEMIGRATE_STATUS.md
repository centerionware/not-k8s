...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36134496060](https://github.com/centerionware/not-k8s/actions/runs/36134496060) passed utility/runtime builds, Docker preflight, source fixtures, Gateway/HTTPRoute, certificate, workload, Secret/ConfigMap, PVC, Job, and CronJob checks. Both lanes then failed because the RBAC test Job heredoc evaluated `$(NODE_NAME)` on the runner. Escaping is fixed in the worktree; neither lane invoked nodemigrate yet. | [CI status](NODEMIGRATE_CI_STATUS.md) |
