...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36130714079](https://github.com/centerionware/not-k8s/actions/runs/36130714079) passed utility/runtime builds and five-node Docker preflight. Fixture CPU requests, seed Pod, Gateway, and nested HTTPRoute condition checks all passed. Both lanes then hit an unset `$stage` in the source fixture's PriorityClass assertion; that variable is now initialized. Neither lane invoked nodemigrate; rerun pending. | [CI status](NODEMIGRATE_CI_STATUS.md) |
