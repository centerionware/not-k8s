...
| Bugs found and component fixes | Confirmed runtime defects are tracked by owning component, including API-version negotiation, CSR ExtraValue codec, controller write identity/RBAC, failed-cutover rollback, omitted standalone Pods/rollout history, and Gateway API CEL compatibility. Both global `matches` and typed map-key CEL fixes pass focused nodeapiserver quick-checks; branch migration retest is running. | [Bug and fix tracker](NODEMIGRATE_BUGS.md) |
...
| K3s/Cilium and upstream Kubernetes/Cilium test lanes | Latest branch-runtime run [36145520889](https://github.com/centerionware/not-k8s/actions/runs/36145520889) passed utility/runtime builds, Docker preflight, and source checks in both lanes. Both invoked nodemigrate; the global CEL `matches` issue was fixed, but imports rolled back on Gateway API map-key CEL validation, with an additional upstream CertificateRequest webhook HTTP 500 while destination networking was unavailable. No target, reverse, or parity checkpoint passed. | [CI status](NODEMIGRATE_CI_STATUS.md) |
