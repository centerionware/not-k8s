...
- Service/Endpoints → **kube-proxy**'s job in stock Kubernetes, and a separate component here too: the `nodeproxy` binary (`crates/nodeproxy/`, nftables). It lived inside nodelet until it was split out into its own binary; either way it isn't a nodelet gap — stays as-is, not touched by this pass.
...
done"). Closes round 88's own documented follow-up: round 88 gave
every `hostUsers: false` pod's regular `volumeMounts` the same
UID/GID `IDMapping` the sandbox itself already gets (round 25), but
left nodelet's own two auxiliary host-bind-mounts — the generated
`/etc/hosts` (`hostAliases`) and the `terminationMessagePath` host
file — unmapped, since they're built outside `build_mounts()`
entirely (`create_and_start_container()` pushes them onto `mounts`
directly, after `build_mounts()` returns).

`mount_id_mappings()` (`volumes_pure.rs`) made `pub(crate)` so
`container_create.rs` can reuse it directly rather than duplicating
the `IdMapping { host_id, container_id: 0, length }` construction —
same `userns_mapping` input `build_mounts()` already receives,
computed once (`aux_id_mappings`) and applied to both auxiliary
`Mount`s. No new RPC, no new allocation — this is exactly the same
already-resolved range `run_sandbox()` claimed for the pod, read back
via `UsernsAllocator::assigned()`, same as round 88's own volumeMounts
path.

New `test_host_aliases_still_work_under_host_users_false`
(`deploy/lib/test/cases/volumes.sh`) — fully automated proof the
`/etc/hosts` mount still works with the new mapping applied
(`hostAliases` entries still land correctly under `hostUsers: false`).
The manual-note test's text extended with the specific ownership-
translation spot-check for both auxiliary mounts (same pre-chowned-
host-file, root-required setup the existing volumeMounts spot-check
already needs).

## Round 97: `volumeMounts[].recursiveReadOnly: IfPossible` real fallback (2026-08-01, same day)

With all 3 of the user's chosen ❌ items done (rounds 94-96), the
user asked to work through the remaining 🟡 partial/simplified
items too ("do them all, let's get it done"). Of the 9, 4 are
genuinely closeable by writing more code; the other 5 are DRA/CSI
items whose only remaining gap is "unvalidated against a real
driver" — a hardware/infra limitation, not missing code (see the
persistent memory file for the full breakdown). This round closes
the first of the 4: round 85's documented `IfPossible`-treated-
identically-to-`Enabled` simplification.

`recursive_read_only_cri()` (`volumes_pure.rs`) now takes a new
`handler_supports_recursive_ro: bool` and makes a real decision:
`Enabled` still always asks for it (unchanged — an unsupporting
runtime rejects/ignores per its own CRI implementation); `IfPossible`
resolves to `true` only when the resolved `RuntimeClass` handler
actually advertises `recursiveReadOnlyMounts` support, `false`
(falling back to a plain read-only mount) otherwise. The capability
comes from a new `CriRuntime.recursive_read_only_handlers: HashMap<String,
bool>` cached once at `connect()` time from the same `Status` RPC
`runtime_handlers()` already makes on demand for
`Node.status.runtimeHandlers` (round 53) — no extra RPC per container
creation, matching this codebase's long-standing "cache once, don't
add RPCs to the hot per-container path" posture (round 90/91's own
container-user/recursiveReadOnly-reporting work took the same
approach). `runtime_handler` (already resolved once per pod reconcile
via `resolve_runtime_handler()`) is threaded down through
`ensure_container()`/`ensure_ephemeral_container()`/
`ensure_init_containers()` → `create_and_start_container()` →
`build_mounts()`/`volume_mount_status_tuples()`, so the *reported*
`containerStatuses[].volumeMounts[].recursiveReadOnly` status matches
exactly what was actually requested of CRI, same "one resolution, used
consistently at request time and report time" discipline round 91
established.

Existing unit tests updated for the new signature (`recursive_read_only.rs`,
`volume_mount_status_tuples.rs`, `mounts.rs`) plus new assertions
proving `IfPossible` now genuinely diverges from `Enabled` when the
handler doesn't support it. New
`test_recursive_read_only_if_possible_falls_back_without_erroring` in
`deploy/lib/test/cases/volumes.sh` — fully automated (a pod reaches
Running either way and its reported status is always a valid value,
regardless of which way this test cluster's own runtime resolves the
capability); the manual-note test's text extended with the specific
Enabled-vs-Disabled-matches-advertised-capability spot-check, which
does need to know the runtime's actual capability ahead of time.
...
"same deal, do both then another audit" instruction — this closes the
`mountPropagation`/`recursiveReadOnly` pair for the second time this
project has hit exactly this shape of 2-item backlog.

- New pure `recursive_read_only_cri(recursive_read_only: Option<&str>,
  readonly: bool, propagation: MountPropagation) -> bool`
  (`volumes_pure.rs`) — translates the API's own ternary (`Disabled`/
  `IfPossible`/`Enabled`; `None` treated as `Disabled`, matching real
  kubelet's own documented "nil is just treated as false") into CRI's
  plain boolean `Mount.recursive_read_only`, per CRI's own proto
  comment: *"when set to true, readonly must be explicitly set to true,
  and propagation must be PRIVATE."* Checked defensively here (never
  sends a contract-violating combination to CRI) rather than trusted
  from the caller — the same posture this codebase already takes toward
  other CRI-level invariants like `Mount.image`/`Mount.host_path`'s
  mutual exclusivity.
- **Documented scope simplification, not silently wrong**: `IfPossible`
  ...
  need the resolved runtime handler's own `recursiveReadOnlyMounts`
  capability — already surfaced to `Node.status.runtimeHandlers` since
  round 53, but not threaded down into this per-mount translation path
  this round (that plumbing would mean passing the runtime-handler
  lookup, currently only made once per pod at sandbox-creation time,
  all the way down into `build_mounts()`'s otherwise-pure per-container
  call). An unsupporting runtime is left to reject or ignore the
  request per its own CRI implementation, matching how this codebase
  already treats an unsupported `sysctl` (round 41) — not a regression
  from any prior behavior, since the field was never set at all before
  this round either way.
- Wired into both branches of `build_mounts()`, mirroring round 84's
  `mountPropagation` wiring exactly (computed once per `volumeMounts`
  entry, applied to both the `HostPath` and image-backed `Image` `Mount`
  variants).
- 10 new unit tests: `recursive_read_only_cri()`'s own matrix (7:
  `Enabled`/`IfPossible` both true under valid preconditions,
  `Disabled`/`None`/unrecognized all false, and the 2 CRI-contract
  precondition-violation cases) plus 3 in `mounts.rs` proving the field
  is actually threaded through `build_mounts()`'s output.
- New e2e test `test_recursive_read_only_still_mounts_read_only_normally`
  (`volumes.sh`): proves the ordinary top-level read-only behavior still
  works with `recursiveReadOnly: Enabled` set (the "didn't break the
  common case" regression check, same reasoning round 84's sibling test
  used). New manual-note documents the real recursiveness verification
  (needs a second `mount(2)` syscall performed inside the container's
  own mount namespace before the outer mount is made — genuinely can't
  be automated by this harness).

## Round 84: volumeMounts[].mountPropagation (2026-07-31, same day)

Closes the higher-value of round 83's 2 findings, per the user's "same
deal, do both then another audit" instruction — `recursiveReadOnly` is
round 85.

- New pure `mount_propagation_cri(mount_propagation: Option<&str>) ->
  MountPropagation` (`volumes_pure.rs`) — `None`/unset and any
  unrecognized value both fall back to `PropagationPrivate`, the same
  CRI zero-value default this codebase already produced by omission
  before this round (so every mount that never set the field is
  behaviorally unchanged), matching the same fail-safe direction
  `protocol_cri()` (round 82) already established for an analogous
  unrecognized-value case.
- Prost's generated enum keeps the proto's own `PROPAGATION_` prefix on
  every variant (`PropagationPrivate`/`PropagationHostToContainer`/
  `PropagationBidirectional`), a naming quirk worth remembering — the
  first build attempt used bare `Private`/`HostToContainer`/`Bidirectional`
  and failed with `E0599`, fixed by checking the actual generated
  `runtime.v1.rs` output.
- Wired into **both** branches of `build_mounts()` (the pre-existing
  `HostPath` mount, and the image-backed `Image` mount from round 32) —
  computed once per `volumeMounts` entry alongside the existing `subPath`
  resolution, since both `Mount` variants share the same field.
- 8 new unit tests: `mount_propagation_cri()`'s own matrix (5: unset,
  both real values, explicit `"None"` string, unrecognized-value
  fallback) plus 3 in `mounts.rs` proving the field is actually threaded
  through `build_mounts()`'s output, not just correct in isolation.
- New e2e test `test_mount_propagation_host_to_container_still_mounts_normally`
  (`volumes.sh`): proves wiring this newly-touched field on *every*
  mount (not just ones that set it) didn't break the ordinary case — a
  real risk specifically because this round changes behavior for every
  single mount's construction path, not just an opt-in feature. New
  manual-note documents the actual live-propagation verification
  procedure (a real `mount(2)` syscall on the host, after the pod is
  already running, root required) — the same honest-limitation
  treatment this project gives anything needing genuine host-level
  mount operations mid-test that this bash-only harness won't do to a
  live node automatically.
...
- ✅ **Dynamic Resource Allocation** (`spec.resourceClaims`; found in round 58's re-audit; closed round 63, gating + batching closed round 64, verified live + 3 real bugs fixed round 121) — kubelet's actual responsibilities are implemented: resolving a pod's `resourceClaims` to their `ResourceClaim` objects' `status.allocation`, gating on `status.reservedFor` actually listing this pod (round 64 — real kubelet's own safety check; `reservedFor` itself is scheduler-written, not kubelet-written, correcting round 63's docs), calling the owning driver(s)' `NodePrepareResources`/`NodeUnprepareResources` once per driver per pod covering every claim it owns (round 64 batching; `dra.rs`, reusing the plugin-registration infrastructure with a new `"DRAPlugin"` type), and wiring the returned CDI device IDs into each container's CRI `ContainerConfig.cdi_devices` per its `resources.claims[].{name, request}` entries. **Round 121**: stood up `kubernetes-sigs/dra-example-driver` (the reference driver real Kubernetes e2e/conformance tests use for this, same role csi-driver-host-path plays for CSI) and found/fixed 3 real bugs that had made the whole feature non-functional end-to-end despite looking complete on inspection — see `docs/E2E_FINDINGS.md` finding #18 for the full story: (1) projected ServiceAccount tokens never bound to their Pod (`bound_object_ref` hardcoded `None`), (2) `ResourceClaim` fetched against `resource.k8s.io/v1beta1`, which doesn't exist on any cluster ≥1.34, (3) `proto/draplugin.proto`'s reconstructed wire format was genuinely wrong (service package name and `Device` field layout both incorrect) — now transcribed directly from upstream `k8s.io/kubelet/pkg/apis/dra/v1/api.proto`. Verified live: a real Pod gets real CDI-injected env vars from the reference driver's `NodePrepareResources` response. `docs/GAP_CLOSURE.md`'s own earlier "unvalidated against a live third-party driver" caveat is resolved.
...
### Security context
- ✅ **`securityContext`** — `runAsUser`/`runAsGroup`, capabilities add/drop, `privileged`, `readOnlyRootFilesystem`, `allowPrivilegeEscalation`→`no_new_privs`, `supplementalGroups`, `seccompProfile`, and AppArmor profiles (`linux_security_context()`). Still not implemented: `runAsNonRoot` verification against the image's actual user and SELinux options.
...
- ✅ **Per-volume `Mount.uidMappings`/`.gidMappings`** (round 88, found in round 86's re-audit; extended to auxiliary host-bind-mounts round 98) — every `HostPath`-shaped volume mount for a `hostUsers: false` pod carries the same UID/GID `IDMapping` (`host_id`/`container_id: 0`/`length`) `run_sandbox()` already applies at the sandbox level (round 25), via pure `mount_id_mappings()` (now `pub(crate)`, round 98) and `UsernsAllocator::assigned()`'s read-only lookup (no re-allocation — just reads back the range `run_sandbox()` already claimed for this pod uid). Without this, kernel-level idmapped-mounts translation (Linux 5.12+) only applies to mounts CRI is actually told to map — a file the host sees as owned by some in-range UID would otherwise show up wrong (`nobody`/overflow, or untranslated) inside the container's own view of that volume. Image-backed mounts (round 32) are still out of scope — they never go through the host bind-mount path idmapping applies to at all. Round 98 closed the one remaining gap: nodelet's own auxiliary host-bind-mounts (`hostAliases`' `/etc/hosts`, `terminationMessagePath`) now carry the same mapping too (`container_create.rs`'s `aux_id_mappings`), so every host-bind-mount a `hostUsers: false` pod gets — not just its declared `volumeMounts` — is consistently id-mapped. Genuinely automated e2e tests prove this doesn't break the mount itself (every regular volumeMount, and now `hostAliases` combined with `hostUsers: false`); the actual ownership-translation behavior needs a host-side file pre-chowned into the pod's specific mapped range before the pod starts (root required) — a manual spot-check procedure.
...
- ✅ Service/ClusterIP/NodePort routing (nftables — kube-proxy's job, reimplemented here as the separate `nodeproxy` binary)
...
- ✅ **`volumeMounts[].recursiveReadOnly`** (round 85, GA 1.33, KEP-3116, found in round 83's re-audit; `IfPossible` fallback closed round 97) — pure `recursive_read_only_cri()` translates the API's ternary (`Disabled`/`IfPossible`/`Enabled`, `None` treated as `Disabled`) into CRI's plain boolean `Mount.recursive_read_only`, defensively enforcing the CRI proto's own contract (`recursive_read_only: true` requires `readonly: true` and `propagation` to resolve to `Private`) — never sends a contract-violating combination, silently downgrading to `false` instead. `Enabled` always asks for it (an unsupporting runtime is left to reject/ignore, same treatment this codebase already gives an unsupported `sysctl`, round 41); `IfPossible` (round 97) is a real best-effort fallback — `true` only when the resolved `RuntimeClass` handler actually advertises `recursiveReadOnlyMounts` support (`Node.status.runtimeHandlers`, round 53; `CriRuntime.recursive_read_only_handlers` caches this from a one-time `Status` RPC at `connect()`, no extra RPC per container), `false` (plain read-only mount) otherwise.
...
- [x] Round 84: `volumeMounts[].mountPropagation` — new pure
      `mount_propagation_cri()` maps `HostToContainer`/`Bidirectional`
      onto CRI's `MountPropagation` enum, wired into both branches of
      `build_mounts()`; unset/unrecognized fall back to the same
      `PRIVATE` default this codebase already produced by omission, so
      existing mounts are unaffected. 8 new unit tests + a genuinely
      automated e2e test proving the common (unset) case still works,
      plus a manual-note for live propagation verification (needs a real
      host-side `mount(2)`). See round 84 notes.
- [x] Round 85: `volumeMounts[].recursiveReadOnly` — new pure
      `recursive_read_only_cri()` translates the API's ternary into CRI's
      plain boolean, defensively enforcing the proto's own
      `readonly: true` + `Private` propagation contract. `IfPossible`
      currently treated identically to `Enabled` (documented scope
...
- [x] Round 97: `volumeMounts[].recursiveReadOnly: IfPossible` real
      fallback — closes round 85's documented simplification.
      `handler_supports_recursive_ro`, cached once from `Status` RPC
      results at `connect()` (no extra RPC per container), threaded
      through `ensure_container()`/`ensure_ephemeral_container()`/
      `ensure_init_containers()` → `build_mounts()`/
      `volume_mount_status_tuples()`. First of 4 closeable 🟡 items
      (user: "do them all, let's get it done" — the other 5 🟡 items
      are DRA/CSI-driver-validation gaps, not missing code). See round
      97 notes.
- [x] Round 98: `hostAliases`/`terminationMessagePath` id-mapping —
      closes round 88's own documented follow-up. `mount_id_mappings()`
      made `pub(crate)`, reused directly in `container_create.rs` for
...
## Nodemigrate migration follow-up (2026-09-25)

- AppArmor CRI propagation is implemented in `linux_security_context()`:
  Kubernetes pod- and container-level profiles map to CRI `SecurityProfile`,
  with container-level override and Localhost profile-name preservation.
  Unexpected types retain `RuntimeDefault` confinement. Five focused
  regressions cover pod propagation, override, Localhost, unset, and an
  unexpected type. The defect was observed in migration run
  [36162293718](https://github.com/centerionware/not-k8s/actions/runs/36162293718),
  where Cilium's `mount-cgroup` logged `nsenter: cannot open
  /hostproc/1/ns/cgroup: Permission denied` while CRI showed the runtime
  default AppArmor profile despite the Pod requesting `Unconfined`. Focused
  nodelet CI and branch-runtime migration verification are pending.
