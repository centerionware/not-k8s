...
| --- | --- | --- |
| Configuration and identity | Namespaces, ConfigMaps (including binary data), Secrets, ServiceAccounts, Roles, ClusterRoles, RoleBindings, ClusterRoleBindings, ResourceQuotas, LimitRanges, and PriorityClasses; verify identity/data and allowed plus denied requests using real service-account credentials. | The fixture now includes binary/text ConfigMaps, a Secret, ServiceAccount/Role/RoleBinding, and Jobs using a real service-account token to verify permitted ConfigMap reads and denied Secret reads. Focused behavior and round-trip verification are pending; quota, limit, and cluster-wide RBAC matrix coverage remains incomplete. |
...
| Helm-managed applications | Helm charts/releases and release records (name, namespace, chart/version, values, revision, manifest), plus all chart-managed resources; verify `helm list`, release inspection, workload health, and a safe follow-up Helm operation after each cutover. | Traefik and cert-manager are installed by Helm. The fixture now inspects release values, manifests, and history and performs a version-pinned server-side dry-run upgrade of Traefik at each checkpoint; these new checks are pending migration CI. |
