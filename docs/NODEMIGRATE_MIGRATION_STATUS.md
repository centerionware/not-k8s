...
| Replace a migrating control-plane Node object | Forward migration requires `NODEMIGRATE_REPLACE_NODE=true` when the source or joined destination already has the local node name; after import/join it deletes that stale Node before starting the node agent and waiting for fresh Ready. Reverse migration applies the same explicit check and deletes the stale object after target API readiness/import. A staged reverse cutover returns before this point, so its coordinator must verify fresh registration after quorum returns. The integration lane opts in for both directions. | Source/destination replacement validation passed in [run 35966523279](https://github.com/centerionware/not-k8s/actions/runs/35966523279). No live registration or replacement-member runtime evidence. |
