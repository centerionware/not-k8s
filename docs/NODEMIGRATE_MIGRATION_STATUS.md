...
| Replace a worker against an existing nodestore cluster | Worker path avoids cluster-wide object import, requires a joined destination, explicitly guards same-name Node removal, waits for fresh Ready registration, and snapshots local hostPath/local PV data from the destination PV inventory | Focused code fixtures are in place; new per-worker snapshot path awaits targeted CI. Real worker cutover, data restoration, rollback, and uninstall behavior remain unverified. |
