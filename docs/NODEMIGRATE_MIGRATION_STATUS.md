...
In branch-runtime run [36082829012](https://github.com/centerionware/not-k8s/actions/runs/36082829012), both K3s and kubeadm forward migrations reached destination API readiness but failed post-cutover workload checks. Nodelet logs identify a startup deadlock: the CoreDNS readiness gate paused the normal Pod watch, while Cilium's host-network agent Pods remained `Unknown`; containerd repeatedly reported `cni plugin not initialized`, CoreDNS never became healthy, and CSI StatefulSet Pods stayed `Terminating`. Nodelet now reconciles local host-network Pods during the gate so an external CNI agent can start before CoreDNS. A focused nodelet check and branch-runtime migration rerun are pending. The migration test captures nodelet journal, CRI state, Pod YAML, and events when fixture setup fails. Forward workload verification, reverse migration, and semantic comparison remain pending.
