...
| Return a multi-control-plane cluster when either datastore needs peers to reach quorum | `stage-target=true` exports nodestore state, disables the local nodestore stack, starts the retained target control plane, and returns before API readiness; later CP can import after target quorum forms. `skip-api-export=true` supports the final CP after import, requiring a ready destination API and backing up local PV data from that API. Both options require CP roles at both ends. | Focused CI pending; no three-CP runtime evidence. This is an operator-ordered protocol, not an automatic cluster coordinator. |
