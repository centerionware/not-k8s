...
used `bd3b4f8f4fae50af73fab88e01a4752f06e01640`. Quick-check
[36247102622](https://github.com/centerionware/not-k8s/actions/runs/36247102622)
passed the new nodeapiserver Job-selector unit tests. Both migration lanes
passed CronJob Job selection, RBAC Jobs, certificate readiness, and earlier
target workload checks. They then failed waiting for the migrated static PVC
to become Bound; no return migration ran. The target import strips object
`status` but keeps the source `pv.kubernetes.io/bind-completed` annotation.
The PV binder considered `spec.volumeName` plus that stale annotation fully
bound, so it never restored the missing `status.phase=Bound`. A focused
nodecontroller fix now also requires the Bound status phase before skipping
reconciliation; its unit regression and migration rerun are pending. Logs and
artifacts: `/tmp/nodemigrate-36247102741-{k3s,kubernetes}-job.log` and
`/tmp/nodemigrate-36247102741-artifacts/`.
