...
`kubectl describe` showed `IngressClass: traefik` but no host rules; raw source
and target Ingress JSON and the class controller were not captured. This is an
unresolved integration failure, not yet a confirmed component defect. No
return migration or semantic parity checkpoint passed. The test harness now
asserts the exact Ingress rule and IngressClass controller at each stage and
prints both safe JSON specs on failure. PR validation
[36253405752](https://github.com/centerionware/not-k8s/actions/runs/36253405752)
passed; the dedicated diagnostic rerun
[36253413938](https://github.com/centerionware/not-k8s/actions/runs/36253413938)
is in progress. It is not yet evidence of an Ingress fix.
