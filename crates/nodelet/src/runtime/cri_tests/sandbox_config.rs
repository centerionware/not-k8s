...
fn log_directory_matches_the_ns_name_uid_layout() {
    let cfg = sandbox_config(&id("kube-system", "coredns-abc", "uid-1", false), None, "coredns-abc", &HashMap::new(), None, false);
...
fn non_host_network_pod_honors_an_explicit_resolved_hostname_override() {
    ...
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), None, "custom-host", &HashMap::new(), None, false);
...
fn host_network_pod_leaves_hostname_empty() {
    ...
    let cfg = sandbox_config(&id("ns", "myapp", "u", true), None, "myapp", &HashMap::new(), None, false);
...
fn sandbox_labels_are_attached() {
    let cfg = sandbox_config(&id("ns", "n", "u", false), None, "n", &HashMap::new(), None, false);
...
fn a_userns_mapping_sets_pod_mode_uid_gid_id_mappings() {
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), Some((100_000, 65_536)), "myapp", &HashMap::new(), None, false);
...
fn host_pid_sets_node_pid_namespace() {
    ...
    let cfg = sandbox_config(&pod_id, None, "myapp", &HashMap::new(), None, false);
...
fn sysctls_are_passed_through_to_the_linux_config() {
    ...
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), None, "myapp", &sysctls, None, false);
...
fn sandbox_supplemental_groups_policy(cfg: &PodSandboxConfig) -> i32 {
    cfg.linux.as_ref().and_then(|l| l.security_context.as_ref()).expect("expected security_context to be set").supplemental_groups_policy
}

#[test]
fn no_pod_security_context_defaults_to_merge() {
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), None, "myapp", &HashMap::new(), None, false);
    assert_eq!(sandbox_supplemental_groups_policy(&cfg), v1::SupplementalGroupsPolicy::Merge as i32);
}

#[test]
fn explicit_strict_is_wired_through() {
    let pod_sc = PodSecurityContext { supplemental_groups_policy: Some("Strict".to_string()), ..Default::default() };
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), None, "myapp", &HashMap::new(), Some(&pod_sc), false);
    assert_eq!(sandbox_supplemental_groups_policy(&cfg), v1::SupplementalGroupsPolicy::Strict as i32);
}

#[test]
fn explicit_merge_is_wired_through() {
    let pod_sc = PodSecurityContext { supplemental_groups_policy: Some("Merge".to_string()), ..Default::default() };
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), None, "myapp", &HashMap::new(), Some(&pod_sc), false);
    assert_eq!(sandbox_supplemental_groups_policy(&cfg), v1::SupplementalGroupsPolicy::Merge as i32);
}

// --- privileged (found live: a real CSI driver's privileged sidecars all
// failed CreateContainer with "no privileged container allowed in
// sandbox" — the sandbox itself was never told to expect one) ---

fn sandbox_privileged(cfg: &PodSandboxConfig) -> bool {
    cfg.linux.as_ref().and_then(|l| l.security_context.as_ref()).expect("expected security_context to be set").privileged
}

#[test]
fn privileged_true_is_wired_through_to_the_sandbox_security_context() {
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), None, "myapp", &HashMap::new(), None, true);
    assert!(sandbox_privileged(&cfg), "sandbox itself must be marked privileged, or CRI refuses to create any privileged container inside it later");
}

#[test]
fn privileged_false_is_the_default() {
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), None, "myapp", &HashMap::new(), None, false);
    assert!(!sandbox_privileged(&cfg));
}

// --- pod_requests_privileged() itself ---

fn container(name: &str, privileged: Option<bool>) -> Container {
    Container {
        name: name.to_string(),
        security_context: privileged.map(|p| SecurityContext { privileged: Some(p), ..Default::default() }),
        ..Default::default()
    }
}

#[test]
fn no_containers_is_not_privileged() {
    assert!(!pod_requests_privileged(&[], &[]));
}

#[test]
fn a_container_with_no_security_context_is_not_privileged() {
    assert!(!pod_requests_privileged(&[container("app", None)], &[]));
}

#[test]
fn a_container_with_privileged_false_is_not_privileged() {
    assert!(!pod_requests_privileged(&[container("app", Some(false))], &[]));
}

#[test]
fn a_container_with_privileged_true_makes_the_whole_pod_privileged() {
    assert!(pod_requests_privileged(&[container("app", Some(true))], &[]));
}

#[test]
fn one_privileged_container_among_several_is_enough() {
    let containers = [container("sidecar-a", Some(false)), container("sidecar-b", None), container("sidecar-c", Some(true))];
    assert!(pod_requests_privileged(&containers, &[]));
}

#[test]
fn a_privileged_init_container_counts_too() {
    // Confirmed live against a real CSI driver deployment (kubernetes-csi/
    // csi-driver-host-path): the sandbox is created once, before any
    // container (app or init) starts — an init container needing
    // privileged mode has exactly the same "sandbox must already know"
    // requirement an app container does.
    assert!(pod_requests_privileged(&[], &[container("init-a", Some(true))]));
}
