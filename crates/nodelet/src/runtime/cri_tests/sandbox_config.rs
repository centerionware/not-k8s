...
fn log_directory_matches_the_ns_name_uid_layout() {
    let cfg = sandbox_config(&id("kube-system", "coredns-abc", "uid-1", false), None, "coredns-abc", &HashMap::new(), None);
    assert_eq!(cfg.log_directory, "/var/log/pods/kube-system_coredns-abc_uid-1");
}

#[test]
fn non_host_network_pod_sets_hostname_to_pod_name() {
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), None, "myapp", &HashMap::new(), None);
...
fn non_host_network_pod_honors_an_explicit_resolved_hostname_override() {
    ...
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), None, "custom-host", &HashMap::new(), None);
...
fn host_network_pod_leaves_hostname_empty() {
    ...
    let cfg = sandbox_config(&id("ns", "myapp", "u", true), None, "myapp", &HashMap::new(), None);
...
fn sandbox_labels_are_attached() {
    let cfg = sandbox_config(&id("ns", "n", "u", false), None, "n", &HashMap::new(), None);
...
fn a_userns_mapping_sets_pod_mode_uid_gid_id_mappings() {
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), Some((100_000, 65_536)), "myapp", &HashMap::new(), None);
...
fn host_pid_sets_node_pid_namespace() {
    ...
    let cfg = sandbox_config(&pod_id, None, "myapp", &HashMap::new(), None);
...
fn sysctls_are_passed_through_to_the_linux_config() {
    ...
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), None, "myapp", &sysctls, None);
...
fn explicit_strict_is_wired_through() {
    ...
    let cfg = sandbox_config(&id("ns", "myapp", "u", false), None, "myapp", &HashMap::new(), Some(&pod_sc));
