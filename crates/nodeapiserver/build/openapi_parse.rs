...
/// One schema's `"required"` array flattened to (schema, field) pairs —
/// Group F's `scheme::validation` reads this for structural "is this field
/// present at all" checks. Kept as its own table rather than folded into
/// `FieldMeta` because `required` is a property of the *schema* (a JSON
/// array of field names sitting alongside `properties`), not of the
/// property itself the way `x-kubernetes-*` extensions and `default` are —
/// there is no per-property JSON node to hang it on.
#[derive(Debug, Clone)]
...
/// A field's own declared OpenAPI `"type"` — `"string"`/`"integer"`/
/// `"boolean"`/`"number"`/`"array"`. A field with no `"type"` key at all
/// (a single nested-object field, spelled via `allOf: [{$ref: ...}]`
/// with no sibling `"type"`, confirmed against `PodSpec.securityContext`)
/// has no entry — that shape is already covered by `FieldMeta::ref_schema`
/// recursion instead. Kept as its own table for the same reason
/// `RequiredField` is: this is a much bigger, much less selective set
/// (nearly every scalar/array field carries a `"type"`) than
/// `FIELD_META`'s "only fields with x-kubernetes-*/default/ref metadata"
/// scope, so folding it into `FieldMeta` would make every existing
/// `FIELD_META` entry conflate two different questions ("does this field
/// have interesting patch/SSA metadata" vs "what basic JSON kind does
/// this field's value have").
#[derive(Debug, Clone)]
pub struct TypeInfo {
    pub schema: String,
    pub field: String,
...
pub fn parse_all(root: &Path) -> (Vec<FieldMeta>, Vec<GvkEntry>, Vec<RequiredField>, Vec<TypeInfo>) {
    ...
    let mut types: BTreeMap<(String, String), String> = BTreeMap::new();
        ...
        for (schema_name, schema) in schemas {
            ...
            if let Some(req) = schema.get("required").and_then(Value::as_array) {
                for field in req.iter().filter_map(Value::as_str) {
                    required.insert((schema_name.clone(), field.to_string()));
                }
            }
            ...
            for (field_name, prop) in props {
                ...
                if let Some(t) = prop.get("type").and_then(Value::as_str) {
                    types.insert((schema_name.clone(), field_name.clone()), t.to_string());
                }
            }
        }
    }
    ...
    let required = required
        .into_iter()
        .map(|(schema, field)| RequiredField { schema, field })
        .collect();
    let types = types
        .into_iter()
        .map(|((schema, field), openapi_type)| TypeInfo { schema, field, openapi_type })
        .collect();
    (field_meta.into_values().collect(), gvks, required, types)
}
...
pub fn render(field_meta: &[FieldMeta], gvks: &[GvkEntry], required: &[RequiredField], types: &[TypeInfo]) -> String {
    ...
    let mut sorted_required: Vec<&RequiredField> = required.iter().collect();
    sorted_required.sort_by(|a, b| (a.schema.as_str(), a.field.as_str()).cmp(&(b.schema.as_str(), b.field.as_str())));
    let mut sorted_types: Vec<&TypeInfo> = types.iter().collect();
    sorted_types.sort_by(|a, b| (a.schema.as_str(), a.field.as_str()).cmp(&(b.schema.as_str(), b.field.as_str())));
    ...
    out.push_str("pub struct RequiredField {\n");
    out.push_str("    pub schema: &'static str,\n");
    out.push_str("    pub field: &'static str,\n");
    out.push_str("}\n\n");

    out.push_str("pub static REQUIRED_FIELDS: &[RequiredField] = &[\n");
    for r in &sorted_required {
        out.push_str(&format!("    RequiredField {{ schema: {:?}, field: {:?} }},\n", r.schema, r.field));
    }
    out.push_str("];\n\n");

    out.push_str("pub struct TypeInfo {\n");
    out.push_str("    pub schema: &'static str,\n");
    out.push_str("    pub field: &'static str,\n");
    out.push_str("    pub openapi_type: &'static str,\n");
    out.push_str("}\n\n");

    out.push_str("pub static TYPE_INFO: &[TypeInfo] = &[\n");
    for t in &sorted_types {
        out.push_str(&format!(
            "    TypeInfo {{ schema: {:?}, field: {:?}, openapi_type: {:?} }},\n",
            t.schema, t.field, t.openapi_type
        ));
    }
    out.push_str("];\n");
