...
pub fn build_status(username: &str, uid: Option<&str>, groups: &[String]) -> Value {
    let mut user_info = serde_json::json!({"username": username, "groups": groups});
    if let Some(uid) = uid {
