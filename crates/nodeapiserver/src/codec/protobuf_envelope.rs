...
pub fn wrap_unknown(api_version: &str, kind: &str, object_bytes: &[u8]) -> Vec<u8> {
    ...
    unknown.insert(
        "raw".to_string(),
        Value::String(base64_encode(object_bytes)),
    );

    let unknown_bytes = encode_message(UNKNOWN_MESSAGE, &Value::Object(unknown)).expect(
        "encoding runtime.Unknown itself never fails: every field is a known scalar/message type",
    );
...
pub fn unwrap_unknown(bytes: &[u8]) -> Result<(String, String, Vec<u8>)> {
    ...
    let type_meta = decoded
        .get("typeMeta")
        .cloned()
        .unwrap_or(Value::Object(Map::new()));
    let api_version = type_meta
        .get("apiVersion")
    ...
    let raw_b64 = decoded
        ...
        .and_then(Value::as_str)
        .unwrap_or_default();
    let object_bytes = base64_decode(raw_b64)
        .map_err(|e| Error::InvalidBase64("runtime.Unknown.raw".to_string(), e))?;
