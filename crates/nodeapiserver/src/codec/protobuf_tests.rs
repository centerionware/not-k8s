...
mod tests {
    ...
    fn a_simple_object_meta_round_trips() {
        ...
        assert_eq!(
            decoded.get("labels").unwrap(),
            &json!({"app": "web", "tier": "frontend"})
        );
    }
    ...
    fn int_or_string_encodes_a_number_with_no_type_tag_written() {
        ...
        let bytes = encode_int_or_string(
            "ServicePort",
            &ProtoField {
                message: "ServicePort",
                ...
                repeated: false,
                map: false,
                proto_type: "k8s.io.apimachinery.pkg.util.intstr.IntOrString",
            },
            &json!(8443),
        )
        .unwrap();
    ...
    fn quantity_round_trips_a_real_binary_si_value() {
        let field = ProtoField {
            ...
            repeated: false,
            map: false,
            proto_type: "k8s.io.apimachinery.pkg.api.resource.Quantity",
        };
        let bytes = encode_quantity("ResourceFieldSelector", &field, &json!("1.5Gi")).unwrap();
        assert_eq!(
            decode_quantity_message("divisor", &bytes).unwrap(),
            json!("1.5Gi")
        );
    }
    ...
    fn an_absent_additional_properties_key_stays_absent_after_a_round_trip() {
        ...
        let decoded = decode_message(message, &encoded).unwrap();
        assert_eq!(
            decoded.get("additionalProperties"),
            None,
            "a key the client never submitted at all must not appear after a round trip"
        );
    }
    ...
    fn a_negative_int64_round_trips() {
        ...
        let decoded = decode_message(message, &encoded).unwrap();
        assert_eq!(
            decoded.get("terminationGracePeriodSeconds").unwrap(),
            &json!(-1)
        );
    }
    ...
    fn null_fields_are_omitted_not_encoded_as_present() {
        ...
        let decoded = decode_message(message, &encoded).unwrap();
        assert!(
            decoded.get("namespace").is_none(),
            "a null field must not round-trip as present"
        );
    }
    ...
    fn schema_for_gvk_resolves_core_v1_pod() {
        assert_eq!(
            schema_for_gvk("", "v1", "Pod"),
            Some("io.k8s.api.core.v1.Pod")
        );
        assert_eq!(
            schema_for_gvk("apps", "v1", "Deployment"),
