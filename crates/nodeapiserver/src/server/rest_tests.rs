...
mod tests {
    ...
    fn continue_token_round_trips_the_resume_key_and_revision() {
        ...
        let (key, revision) =
            decode_continue_token(&token).expect("a token this module encoded must decode");
    ...
    fn resolve_kind_finds_a_real_known_resource() {
        ...
        assert_eq!(
            resolve_kind("apps", "v1", "deployments"),
            Some("Deployment")
        );
    }
    ...
    fn resolve_kind_finds_namespaced_rbac_resources() {
        for (resource, kind, schema) in [
            ...
            assert_eq!(
                resolve_kind("rbac.authorization.k8s.io", "v1", resource),
                Some(kind)
            );
            assert_eq!(
    ...
    fn decode_stored_object_round_trips_a_real_encoded_object() {
        let schema = protobuf::schema_for_gvk("", "v1", "Namespace")
            .expect("core/v1 Namespace should be a known schema");
    ...
    fn patch_kind_for_content_type_recognizes_all_three_real_media_types() {
        assert_eq!(
            patch_kind_for_content_type("application/json-patch+json"),
            Some(PatchKind::Json)
        );
        assert_eq!(
            patch_kind_for_content_type("application/merge-patch+json"),
            Some(PatchKind::Merge)
        );
        assert_eq!(
            patch_kind_for_content_type("application/strategic-merge-patch+json"),
            Some(PatchKind::StrategicMerge)
        );
    }
    ...
    fn patch_kind_for_content_type_ignores_charset_parameters() {
        assert_eq!(
            patch_kind_for_content_type("application/merge-patch+json; charset=utf-8"),
            Some(PatchKind::Merge)
        );
    }
    ...
    fn name_format_violations_is_empty_for_a_resource_with_no_verified_rule() {
        ...
        assert!(
            name_format_violations("", "events", "Not-A-Valid-DNS-Label-But-Unchecked").is_empty()
        );
    }
    ...
    fn name_format_violations_enforces_the_real_dns_subdomain_rule_on_each_verified_resource() {
        for resource in [
            ...
            assert!(
                name_format_violations("", resource, "my-name.example").is_empty(),
                "{resource} should accept a valid DNS subdomain"
            );
            assert!(
                !name_format_violations("", resource, "My_Bad_Name").is_empty(),
                "{resource} should reject an invalid DNS subdomain"
            );
        }
    }
    ...
    fn protobuf_request_decodes_a_built_in_object_envelope() {
        let schema = protobuf::schema_for_gvk("", "v1", "ConfigMap")
            .expect("ConfigMap has a generated schema");
        let encoded = protobuf::encode_message(
            schema,
            &json!({
    ...
    fn protobuf_request_rejects_a_kind_that_does_not_match_the_resource() {
        ...
        assert!(matches!(
            decode_protobuf_object(&resolved, "configmaps", &envelope),
            Err(Error::InvalidProtobufRequest(_))
        ));
    }
}
