...
pub fn scale_from_parent(parent: &Value) -> Result<Value, String> {
    ...
    for field in ["namespace", "uid", "resourceVersion"] {
        if let Some(value) = parent.pointer(&format!("/metadata/{field}")).filter(|value| !value.is_null()) {
            metadata.insert(field.to_string(), value.clone());
        }
    }

    let replicas = parent.pointer("/spec/replicas").and_then(Value::as_i64).unwrap_or(1);
    let status_replicas = parent.pointer("/status/replicas").and_then(Value::as_i64).unwrap_or(0);
    let mut status = Map::new();
    status.insert("replicas".to_string(), Value::Number(status_replicas.into()));
...
pub async fn get_scale(storage: &mut StorageClient, group: &str, version: &str, resource: &str, namespace: Option<&str>, name: &str) -> Result<ScaleOutcome, Error> {
    ...
    match get(storage, None, group, version, resource, namespace, name).await? {
        GetOutcome::Found(parent) => Ok(ScaleOutcome::Found(scale_from_parent(&parent).map_err(|error| Error::InvalidProtobufRequest(error))?)),
...
pub async fn update_scale(storage: &mut StorageClient, group: &str, version: &str, resource: &str, namespace: Option<&str>, name: &str, body: &Value, dry_run: bool) -> Result<ScaleOutcome, Error> {
    if !supports_scale(group, version, resource) {
        return Ok(ScaleOutcome::UnknownResource);
    }
    ...
    let current_rv = current.pointer("/metadata/resourceVersion").and_then(Value::as_str).unwrap_or("");
    ...
    match update_with_options(storage, group, version, resource, namespace, name, &parent, dry_run).await? {
        ...
        UpdateOutcome::NamespaceMismatch => Ok(ScaleOutcome::Invalid(vec!["metadata.namespace: does not match the request URL".to_string()])),
        UpdateOutcome::Invalid(violations) => Ok(ScaleOutcome::Invalid(violations)),
        UpdateOutcome::UnsupportedPatchType => Ok(ScaleOutcome::Invalid(vec!["the Scale update could not be persisted".to_string()])),
    }
}
...
pub async fn patch_scale(storage: &mut StorageClient, group: &str, version: &str, resource: &str, namespace: Option<&str>, name: &str, kind_of_patch: PatchKind, patch_doc: &Value, dry_run: bool) -> Result<ScaleOutcome, Error> {
    if !supports_scale(group, version, resource) {
        return Ok(ScaleOutcome::UnknownResource);
    }
    let current = match get(storage, None, group, version, resource, namespace, name).await? {
    ...
    match update_with_options(storage, group, version, resource, namespace, name, &parent, dry_run).await? {
        ...
        UpdateOutcome::NamespaceMismatch => Ok(ScaleOutcome::Invalid(vec!["metadata.namespace: does not match the request URL".to_string()])),
        UpdateOutcome::Invalid(violations) => Ok(ScaleOutcome::Invalid(violations)),
        UpdateOutcome::UnsupportedPatchType => Ok(ScaleOutcome::Invalid(vec!["the Scale patch could not be persisted".to_string()])),
    }
}

fn scale_resource_version(value: &Value) -> Option<&str> {
    value.pointer("/metadata/resourceVersion").and_then(Value::as_str).filter(|version| !version.is_empty())
}
...
fn scale_metadata_violations(value: &Value, namespace: Option<&str>, name: &str) -> Vec<String> {
    ...
    if let Some(body_namespace) = metadata.get("namespace") {
        if let Some(namespace) = namespace {
            if body_namespace.as_str().is_some_and(|body_namespace| !body_namespace.is_empty() && body_namespace != namespace) {
...
fn parent_with_replicas(current: &Value, replicas: i64) -> Result<Value, String> {
    ...
    let previous = parent.pointer("/spec/replicas").and_then(Value::as_i64).unwrap_or(1);
    parent
        .get_mut("spec")
    ...
    if previous != replicas {
        let generation = parent.pointer("/metadata/generation").and_then(Value::as_i64).unwrap_or(0);
        set_metadata_field(&mut parent, "generation", Value::Number((generation + 1).into()));
    }
    Ok(parent)
}

fn apply_scale_patch(kind_of_patch: PatchKind, existing: &Value, patch_doc: &Value) -> Result<Value, String> {
    match kind_of_patch {
        PatchKind::Json => {
            let mut value = existing.clone();
            crate::patch::json_patch::apply(&mut value, patch_doc).map_err(|_| "the submitted JSON Patch could not be applied".to_string())?;
            Ok(value)
        }
        PatchKind::Merge => {
            let mut value = existing.clone();
        ...
        PatchKind::StrategicMerge => {
            let schema = protobuf::schema_for_gvk("autoscaling", "v1", "Scale").ok_or_else(|| "autoscaling/v1 Scale has no known schema".to_string())?;
            Ok(crate::patch::strategic_merge::apply(schema, existing, patch_doc))
        }
    }
}
...
fn scale_selector(parent: &Value) -> Option<String> {
    ...
    if let Some(labels) = selector.get("matchLabels").and_then(Value::as_object) {
        ...
        requirements.extend(entries.into_iter().filter_map(|(key, value)| value.as_str().map(|value| format!("{key}={value}"))));
    }
    if let Some(expressions) = selector.get("matchExpressions").and_then(Value::as_array) {
        for expression in expressions {
            let Some(key) = expression.get("key").and_then(Value::as_str) else { continue };
...
mod tests {
    ...
    fn scale_representation_uses_parent_spec_status_and_selector() {
        ...
        assert_eq!(scale["metadata"], json!({"name": "web", "namespace": "default", "uid": "uid-1", "resourceVersion": "42"}));
        assert_eq!(scale["spec"]["replicas"], 3);
        assert_eq!(scale["status"], json!({"replicas": 2, "selector": "app=web,tier=frontend"}));
    }
    ...
    fn scale_parent_update_changes_generation_only_when_replicas_change() {
        ...
        assert_eq!(parent_with_replicas(&parent, 2).unwrap()["metadata"]["generation"], 5);
    }
