...
//! what it does and doesn't cover. `name_format` lands format checks
//! (`is_dns1123_label`/`is_dns1123_subdomain`/`is_dns1035_label`, qualified
//! metadata names, and label values, faithful ports of real upstream's own
//! validators) — wired to the verified resource-name rules and every generic
//! write path's universal metadata checks. Extending the hand-maintained
//! resource-name mapping remains separate work; see that module's own doc
//! comment. `quantity`
