...
//! and the default (no-override) key layout. `resourceVersion == nodestore's
//! MVCC revision` (finding 3) and optimistic-concurrency-via-`Txn` are real
//! today. **Not yet landed, and deliberately not guessed at**: the
//! per-resource key-prefix override table (`keys`'s own doc comment) and
