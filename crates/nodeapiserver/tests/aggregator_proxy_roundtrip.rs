...
async fn an_aggregated_apiservice_resolves_and_relays_through_a_real_tls_backend() {
    ...
    // request.
    let resolved = route::resolve(&mut storage, "metrics.k8s.io", "v1beta1", None).await.expect("resolving the APIService").expect("a matching, non-local APIService must be found");
    assert_eq!(resolved["spec"]["service"]["name"], "metrics-server");
    assert!(route::resolve(&mut storage, "metrics.k8s.io", "v1", None).await.expect("resolving a non-matching version").is_none(), "a different version must not match");
    ...
    let discoverable = route::discoverable_group_versions(&mut storage, None).await.expect("listing discoverable aggregated group-versions");
