...
# at release v0.6.0. Fetches deploy.tar.gz from that release's GitHub
...
echo "==> Fetching https://github.com/centerionware/not-k8s/releases/download/v0.6.0/deploy.tar.gz..." >&2
if command -v curl &>/dev/null; then
    curl -fsSL "https://github.com/centerionware/not-k8s/releases/download/v0.6.0/deploy.tar.gz" -o "$TARBALL"
elif command -v wget &>/dev/null; then
    wget -q -O "$TARBALL" "https://github.com/centerionware/not-k8s/releases/download/v0.6.0/deploy.tar.gz"
else
    echo "not-k8s installer: need curl or wget to fetch https://github.com/centerionware/not-k8s/releases/download/v0.6.0/deploy.tar.gz" >&2
    exit 1
fi
