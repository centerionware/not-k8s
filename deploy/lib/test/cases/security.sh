...
test_host_users_volume_ownership_translation_manual_note() {
    skip_test "genuinely proving OWNERSHIP TRANSLATION (that a host-side file owned by a UID within the pod's own mapped range shows up as the correct small container-relative UID inside the container, rather than the overflow/nobody UID a mount with no idmapping would show) needs a host-side file pre-chowned into a specific UID before the pod starts -- root required, and needs to read this node's live NODELET_USERNS_BASE_UID value. Manual spot-check: (1) note NODELET_USERNS_BASE_UID (default 100000), (2) create a hostPath directory, chown a file inside it to that exact UID ('chown 100000 <dir>/marker' -- this is the FIRST pod's allocated range base, container-relative UID 0), (3) create a hostUsers: false pod mounting that hostPath directory, (4) confirm 'stat -c %u /hostvol/marker' INSIDE the container reports '0' (proof Mount.uidMappings correctly translated host UID 100000 -> container UID 0) rather than 65534/nobody (proof no translation happened at the mount level, even though the sandbox's own userns_options, round 25, works fine on its own). Round 98 extended the same mapping to nodelet's own auxiliary host-bind-mounts (/etc/hosts, terminationMessagePath); round 123 found (live, from the automated volume-roundtrip test finally reaching this deep in the suite) that leaving these files/dirs at nodelet's own default ownership (real host root) doesn't work -- they're outside the mapped range and the container sees the overflow uid, same failure mode as an un-chowned emptyDir. Both are now chowned to the pod's userns base uid/gid right after nodelet creates them (see chown_userns_base()'s call sites in volumes_resolve.rs and container_create.rs). To spot-check: (5) 'stat -c %u <nodelet-state-dir>/pods/<uid>/etc-hosts' and the termination-log file under NODELET_VOLUME_ROOT should now show NODELET_USERNS_BASE_UID itself (not real host root, uid 0) on disk, while (6) 'stat -c %u /etc/hosts' and 'stat -c %u /dev/termination-log' INSIDE the container should report '0' (translated), not 65534/nobody."
}
