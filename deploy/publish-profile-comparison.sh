#!/usr/bin/env bash
# Small metrics only, never build binaries. A separate publication runner
# receives measurement artifacts; the report combines successful legs.
set -euo pipefail
mode=${1:?leg or report}
data=${2:?data directory}
backend=${3:-}
: "${GH_TOKEN:?}" "${GITHUB_REPOSITORY:?}" "${GITHUB_RUN_ID:?}"
case "$mode:$backend" in leg:notk8s|leg:k8s|leg:k3s|report:) ;; *) exit 2 ;; esac
branch=${PROFILE_RESULTS_BRANCH:-profiling-results}
git check-ref-format --branch "$branch" >/dev/null
work=$(mktemp -d)
gh auth setup-git
relative="comparisons/$GITHUB_RUN_ID-${GITHUB_RUN_ATTEMPT:-1}"
remote="https://github.com/$GITHUB_REPOSITORY.git"
if git ls-remote --exit-code --heads "$remote" "$branch" >/dev/null; then
    git clone --filter=blob:none --no-checkout --depth=1 --single-branch --branch "$branch" "$remote" "$work/results"
    git -C "$work/results" sparse-checkout set --no-cone "/$relative/" /latest-comparison.md
    git -C "$work/results" checkout "$branch"
else
    status=$?
    [[ "$status" == 2 ]] || exit "$status"
    git init "$work/results"
    git -C "$work/results" checkout --orphan "$branch"
    git -C "$work/results" remote add origin "$remote"
fi
dest="$work/results/$relative"
git -C "$work/results" config user.name not-k8s-profiling-bot
git -C "$work/results" config user.email actions@users.noreply.github.com
commit_result() {
if [[ "$mode" == leg ]]; then
    mkdir -p "$dest/$backend"
    cp -a "$data/." "$dest/$backend/"
else
    mkdir -p "$dest/charts"
    cp -a "$data/." "$dest/"
    printf '# Latest stack comparison\n\n[Results](%s/README.md)\n' "$relative" > "$work/results/latest-comparison.md"
fi
git -C "$work/results" add "$relative"
[[ "$mode" != report ]] || git -C "$work/results" add latest-comparison.md
if git -C "$work/results" diff --cached --quiet; then return; fi
git -C "$work/results" commit -m "perf: record $mode comparison ${backend:-charts} run $GITHUB_RUN_ID"
}
commit_result
for attempt in {1..10}; do
    if git -C "$work/results" push origin "HEAD:$branch"; then
        printf '[Comparison data](https://github.com/%s/tree/%s/%s)\n' "$GITHUB_REPOSITORY" "$branch" "$relative" >> "${GITHUB_STEP_SUMMARY:-/dev/stdout}"
        exit 0
    fi
    git -C "$work/results" fetch origin "$branch"
    if ! git -C "$work/results" rebase FETCH_HEAD; then
        git -C "$work/results" rebase --abort
        # The checkout is disposable and clean. Reapply only this result on
        # the latest history, rebuilding the pointer rather than merging it.
        git -C "$work/results" checkout --detach FETCH_HEAD
        commit_result
    fi
    sleep "$((attempt < 6 ? attempt : 5))"
done
exit 1
